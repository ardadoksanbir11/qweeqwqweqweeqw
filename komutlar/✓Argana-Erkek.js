...
const tag = '⥉'   
const Argana = message.guild.roles.cache.find(r => r.id === '795011768572444672') 
const Argana2 = message.guild.roles.cache.find(r => r.id === '795011826839846953')
...
.setAuthor(message.author.tag, message.author.avatarURL({dynamic:true}))
.setDescription(`<a:721033655883005994:739539711319605338> 1. Erkek rolü ayarlanmamış/yanlış id girilmiş kayıt işlemine devam edilemiyor.`)
.setColor('#a22a2a')).then(x => x.delete({timeout: 5000}));
if(!Argana2) return message.channel.send(new MessageEmbed()
.setAuthor(message.author.tag, message.author.avatarURL({dynamic:true}))
.setDescription(`<a:721033655883005994:739539711319605338> 2. Erkek rolü ayarlanmamış/yanlış id girilmiş kayıt işlemine devam edilemiyor.`)
.setColor('#a22a2a')).then(x => x.delete({timeout: 5000}));
